<template>
	<view>
		<use-header :title="ToUserInfo.name" >
		...
		<view class="chat-input pos-f dflex padding-tb-sm padding-left safe-area-inset-bottom w-full bg-drak"
			...
			<view class="use-input dflex-c border-radius-sm padding-lr-sm w-full" >
...
<script>
	...
	export default {
		...
		methods: {
			...
			// 	})
			// },
			msgToScreen(remoteUser,nowUser,text) {
				console.log("构建消息")
				 // 当前用户消息
					let that = this;
				    ...
				    if (nowUser) { // nowUser 表示是否显示当前用户发送的聊天消息，绿色气泡
						...
						let msgFromServer = { "name": remoteUser, "text": text}
						this.messageList.push(msgFromServer);
						console.log("准备存储消息")
						uni.setStorage({
							key: "messageList" + toUser,
					...
					// this.scrollToView = 'chatItem_' + 999 // 滚动至选中序号位置
					this.scrollToBottom();
			      ...
			      if (!this.textMsg) {
			       console.log("请输入内容")
			        ...
			        if (typeof (WebSocket) == "undefined") {
			          ...
			          let message = {from:this.myUserInfo.id, to:toUser, text:this.textMsg, type:1}
			          socket.send(JSON.stringify(message));  // 将组装好的json发送给服务端，由服务端进行转发
						//TODO: 构建消息内容，本人消息
			          // this.messages.push({user: this.myUserInfo.name, text: this.textMsg})
			          this.msgToScreen(null, this.myUserInfo.name, this.textMsg)
			          this.textMsg = '';
			        ...
			        socket.onmessage = function (msg) {
			          ...
			          if (data.users) {  // 获取在线人员信息
			            ...
			            if (data.from == toUser) {
			              ...
			              // 构建消息内容
			              _this.msgToScreen(data.from, null, data.text)
			            }
			          }
			        };
			        //关闭事件
			...
			getData(id){
				...
				//获取目标用户信息
				uni.request({
					url: 'http://localhost:9090/user/GetUserInfoById' + '/' + id, //仅为示例，并非真实接口地址。
				...
				console.log('获取用户未读信息','/'+ this.myUserInfo.id + '/' + '1' + '/' +this.toUserid)
				uni.request({
					url: 'http://localhost:9090/message/getUnMessage/'+ this.myUserInfo.id + '/' + '1' + '/' +this.toUserid, //仅为示例，并非真实接口地址。
				    method:'GET',//请求方式  或GET，必须为大写
					success: res => {
						console.log("构建未读信息消息内容")
						...
						if(res.data.code == 400){
							...
							for(var i = 0 ; i < res.data.data.data.length ; i++){
								console.log(i)
								that.msgToScreen(res.data.data.data[i].from, null, res.data.data.data[i].text)
							}
